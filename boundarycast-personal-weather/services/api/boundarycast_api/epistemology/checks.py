...
def evaluate_knowledge_state(evidence, req):
    ...
    checks = {
        ...
        "evidence_fresh": (_within(official.get("freshness_minutes"), 60) and _within(obs.get("freshness_minutes"), 90)),
        "source_known": bool(official.get("source_name") and obs.get("source_name") and alerts.get("source_name")),
        "observation_distance_acceptable": _within(obs.get("distance_km"), 10),
